...
void GETPad::AddBins(TH2Poly *poly) {
    // Add the bins
    double x[4], y[4];
...
bool GETPad::IsDeadPad(int xId, int yId) {
    return this->IsDeadChan(agetId_[xId][yId], chanId_[xId][yId]);
}
bool GETPad::IsEvenChan(int agetId, int chanId) {
...
bool GETPad::IsDeadChan(int agetId, int chanId) {
    if (chanId < 0 || chanId > 67 || agetId < 0 || agetId > 3) {
        std::cerr << "! Error in GETPad::IsDeadChan" << std::endl;
        std::cerr << "Out of range of agetId or chanId!" << std::endl;
        exit(-1);
    }
